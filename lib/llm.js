...
async function analyzeUserPreferences(message) {
...
For spice levels, ONLY set a specific level if the user explicitly mentions their preferred spice level. If they don't mention a spice level, return null.
Understand various ways users might express their preferred spice level and convert them to standard values:
...
IMPORTANT: 
1. Only use tags and content warnings that exist in the database. If a user mentions something that doesn't exist in the database, try to match it to the closest available option.
2. Only set a specific spice level if the user explicitly mentions their preferred spice level. Otherwise, return null.
3. Only include genres and warnings that the user explicitly mentions or strongly implies.

You must return your response as a JSON object with the following structure:
{
    "spiceLevel": "Sweet" | "Mild" | "Medium" | "Hot" | "Scorching" | "Inferno" | null,
    "genres": ["tag1", "tag2", ...],
